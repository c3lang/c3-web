...
We can see here that if there is no trap on overflow, then we can pick a and b so that the allocated size overflows into a small value, and then some\_value actually will be written out of bounds.
...
What had happened if C instead had did a cast on the unsigned to a signed integer? Well INT\_MAX + 10 results in a large negative number, we then subtract 10 from that value, which results in a value less than INT\_MIN, which in C is undefined behaviour due to the overflow.
...
> [notnullnotvoid](https://handmade.network/m/notnullnotvoid)  
>   
