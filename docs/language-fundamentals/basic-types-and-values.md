...
[Arrays](../language-common/arrays.md) have the format `Type[size]`, so for example: `int[4]`. An array is a type consisting
...
[Slices](../language-common/arrays.md#slice) have the format `Type[]`. Unlike the array, a slice does not hold the values themselves
...
`String` is a
[typedef inline](../language-common/alias.md#typedef-inline) `char[]`, which can implicitly convert to `char[]` when required.

`ZString` is a [typedef inline](../language-common/alias.md#typedef-inline) `char*`.`ZString` is a C compatible null terminated string, which can implicitly convert to `char*` when required.
