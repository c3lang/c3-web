...
## 🎁 Unwrapping an Optional {: #unwrapping-optional }
!!! note
...
or [Rethrow](#rethrow-operator) `!`,
then the variable is automatically [unwrapped](#unwrapping-optional) to a non-Optional:
```c3
fn void? test()
{
...
```
### Using the Rethrow operator `!` to unwrap an Optional value {: #rethrow-operator }

- The Rethrow operator `!` will return from the function with the `Excuse` if the Optional result is empty.
- The resulting value will be [unwrapped](#unwrapping-optional) to a non-Optional.
