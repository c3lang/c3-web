...
### End of a scope
The end of a scope also includes `return`, `break`, `continue` or [rethrow `!`](../language-common/optionals-essential.md#rethrow-operator).
...
!!! note
    [Rethrow `!`](../language-common/optionals-essential.md#rethrow-operator) unwraps the Optional result if present, afterwards the previously Optional variable is a normal variable again, if the Optional result is empty then the Excuse is returned from the function back to the caller.
...
A `defer try` is called at [end of a scope](#end-of-a-scope) when the returned [Optional contained a result](../language-common/optionals-essential.md#what-is-an-optional) value.
...
```
Function returns an [Optional result](../language-common/optionals-essential.md#what-is-an-optional) value,
...
```
Function returns an [Optional Excuse](../language-common/optionals-essential.md#what-is-an-optional),
...
[Optional Excuse](../language-common/optionals-essential.md#what-is-an-optional), and is helpful for logging, cleanup and freeing resources.
...
!!! caution "Pitfalls with `defer` and `defer catch`"
    ...
    This helps to avoid unwanted memory leaks or unwanted resource usage from other code [rethrowing `!`](../language-common/optionals-essential.md#rethrow-operator) before the `defer catch` was even declared.
