...
Keep in mind that there can be multiple modules per file in C3 if multiple [module sections](../language-fundamentals/modules.md#module-sections) are used. In contrast, for a per file name try `$$FILE` or `$$FILEPATH`.
