...
The [`import`](../language-fundamentals/modules.md#importing-modules) statement imports other modules, and we want `printn` which
is in `std::io`.

Next we define a [function](../language-fundamentals/functions.md) which starts with the `fn` keyword followed by the return type. We don't need to return anything, so return `void`. The function name `main` then follows, followed by the function's parameter list, which is empty.
```c3
fn void main() {}
```
...
!!! note
    ...
    For Unix-like OSes there are a few different variants, for example we might declare it as `fn void main(String[] args)`. In that case the parameter "args" contains a [slice](../language-common/arrays.md#slice) of strings, of the program's command line arguments, starting with the name of the program itself.
...
Want more options when compiling? [Check the c3c compiler build options](../build-your-project/build-commands.md).
