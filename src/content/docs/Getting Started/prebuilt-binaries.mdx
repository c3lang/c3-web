...
## Install a linker
You will also need a *linker*, which is a program that C3 uses to join its compiled outputs into one file.
It is likely you already have one. If you get linker errors, you should install a C compiler, which will typically include a linker.
A C compiler is also useful because some C3 packages depend on C code and will need a C compiler.
